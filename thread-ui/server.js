...
app.post('/threads/:id/comments', upload.single('image'), (req, res) => {
    ...
    // Function to get IPv4 address from request
    const getIPv4 = (req) => {
      let ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress || '';
  
      // Sometimes the IP address includes '::ffff:', so we extract IPv4 if it's there
      if (ip.includes('::ffff:')) {
        ip = ip.split(':').pop();
      } else if (ip.includes(',')) {
        // If there are multiple IP addresses (e.g., proxy headers), take the first one
        ip = ip.split(',')[0];
      }
